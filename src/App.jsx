...
const WMO = {
  ...
  3:{label:"Overcast",icon:"☁️"},45:{label:"Fog",icon:"🌫️"},48:{label:"Icy Fog",icon:"🌫️"},
...
function baseOptions({ categories, yLabel="", unit="", colors=["#1e6fba"], type="line", height=220 }){
  return {
    ...
    xaxis:{ categories, labels:{ style:{fontSize:"10px",colors:"#5a7d99"}, rotate:-30, rotateAlways:false }, tickAmount:10 },
    yaxis:{ labels:{ style:{fontSize:"11px",colors:"#5a7d99"}, formatter:v=>v!=null?v.toFixed(1):'' }, title:{ text:yLabel, style:{color:"#5a7d99",fontSize:"11px"} } },
    tooltip:{
...
function ChartCard({ title, hint, children }){
  return(
    <div className="chart-card">
      ...
      {hint && <div className="chart-hint">🔍 Pinch / scroll to zoom · Drag to pan · Toolbar for more</div>}
      {children}
    </div>
  );
}
...
function CurrentPage({ location }){
  ...
  const wmo = WMO[cw?.weathercode]||{label:"Unknown",icon:"🌡️"};
  ...
  const visSeries =[{ name:"Visibility km", data:(h?.visibility||[]).map(v=>v!=null?(v/1000):null) }];
      ...
      {!loading && wData && (<>
        ...
        <div className="weather-hero">
          <div>
            ...
            <div className="hero-temp">{cw?T(cw.temperature):"–"}</div>
            <div className="hero-meta">
              ...
              <span className="hero-meta-item">💨 {cw?.windspeed??"–"} km/h</span>
        ...
        <div className="grid-auto" style={{marginBottom:20}}>
          {[
            {icon:"🌡️",label:"Temp (Current)", val:T(cw?.temperature),              unit:""},
            ...
            {icon:"🌂",label:"Precip Prob Max", val:`${d?.precipitation_probability_max?.[0]??0}`,unit:"%"},
          ].map((m,i)=>(
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      ()=>{ setGeoMsg("Location access denied — using New Delhi as fallback."); setLocation({lat:28.6139,lon:77.2090}); },
      {timeout:9000}
    );
  },[]);
      ...
      <div className="app">
        <nav className="nav">
          ...
          <div className="nav-tabs">
            ...
            <button className={`nav-tab${tab==="historical"?" active":""}`} onClick={()=>setTab("historical")}>
              📊 Historical Analysis
            </button>
          </div>
          <div className="nav-right">Powered by Open-Meteo API</div>
        </nav>
