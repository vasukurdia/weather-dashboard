...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .chart-title { font-size: 0.86rem; font-weight: 600; color: var(--navy); margin-bottom: 4px; }
...
const WMO = {
  ...
  75: { label:"Heavy Snow",     icon:"❄️"  }, 80: { label:"Rain Showers",  icon:"🌦️" },
...
const AQI_LEVELS = [
  ...
  { max:150,      label:"Unhealthy (Sensitive)", cls:"aqi-sensitive", emoji:"😷" },
...
function baseOptions({categories,yLabel="",unit="",colors=["#1e6fba"],type="line",height=220}) {
  return {
    ...
    grid:{borderColor:"#e2edf5",strokeDashArray:4,xaxis:{lines:{show:false}}},
    xaxis:{
      categories,
      labels:{style:{fontSize:"10px",colors:"#5a7d99"},rotate:-30,rotateAlways:false},
      tickAmount:10,
    },
    yaxis:{
      labels:{style:{fontSize:"11px",colors:"#5a7d99"},formatter:(v)=>v!=null?v.toFixed(1):""},
      title:{text:yLabel,style:{color:"#5a7d99",fontSize:"11px"}},
    },
    tooltip:{
      theme:"light",x:{show:true},
      y:{formatter:(v)=>v!=null?`${(+v).toFixed(2)} ${unit}`:"N/A"},
      style:{fontSize:"12px"},
    },
...
async function fetchWeather(lat, lon, date) {
  ...
  if (isArchive) {
    ...
    u.searchParams.set("hourly",   hourlyVars);
...
function ChartCard({title,hint,children}) {
  return (
    <div className="chart-card">
      ...
      {hint&&<div className="chart-hint">🔍 Scroll to zoom · Drag to pan · Toolbar for more options</div>}
      {children}
    </div>
  );
}
...
function CurrentPage({location}) {
  ...
  const T  = (c) => c==null?"–":unit==="C"?`${Math.round(c)}°C`:`${toF(c)}°F`;
  ...
  const displayWind = isToday ? cw?.windspeed         : d?.windspeed_10m_max?.[0];
  ...
  const now = new Date();
  let sunPct=0;
  if(sr&&ss){const tot=ss-sr;const el=Math.min(Math.max(now-sr,0),tot);sunPct=Math.round(el/tot*100);}

  const hLabels  = (h?.time||[]).map(t=>`${new Date(t).getHours()}:00`);
  const aqLabels = (aq?.time||[]).map(t=>`${new Date(t).getHours()}:00`);

  const tempSeries = [{name:`Temp °${unit}`, data:(h?.temperature_2m||[]).map(Tv)}];
  const humSeries  = [{name:"Humidity %",    data:h?.relativehumidity_2m||[]}];
  const precSeries = [{name:"Precipitation", data:h?.precipitation||[]}];
  const visSeries  = [{name:"Visibility km", data:(h?.visibility||[]).map(v=>v!=null?+(v/1000).toFixed(2):null)}];
  ...
  return (
    <div>
      ...
      <div className="location-bar">
        ...
        <span style={{color:"var(--muted)"}}>{location.lat.toFixed(4)}°N, {location.lon.toFixed(4)}°E</span>
      </div>
      ...
      <div className="date-row">
        <div>
          ...
          <input type="date" className="date-input" value={date}
            max={toDate(new Date())} onChange={e=>setDate(e.target.value)}/>
        </div>
        <button className="btn btn-primary" onClick={load}>Fetch Weather</button>
        <div>
          <div className="label">Temperature Unit</div>
          <div style={{display:"flex",gap:0,border:"1.5px solid var(--border)",borderRadius:9,overflow:"hidden",width:"fit-content"}}>
            <button className={`toggle-btn${unit==="C"?" on":""}`} onClick={()=>setUnit("C")}>°C</button>
            <button className={`toggle-btn${unit==="F"?" on":""}`} onClick={()=>setUnit("F")}>°F</button>
          </div>
        </div>
      </div>
      ...
      {!loading && wData && (<>
        ...
        <div className="weather-hero">
          <div>
            <div className="hero-city">📍 {city} · {date}</div>
            <div className="hero-desc">{wmo.label}</div>
            <div className="hero-temp">{T(displayTemp)}</div>
            <div className="hero-meta">
              <span className="hero-meta-item">🔻 {T(d?.temperature_2m_min?.[0])}</span>
              <span className="hero-meta-item">🔺 {T(d?.temperature_2m_max?.[0])}</span>
              <span className="hero-meta-item">💧 {curHum??"–"}%</span>
              <span className="hero-meta-item">💨 {displayWind??"–"} km/h</span>
              <span className="hero-meta-item">🌧️ {d?.precipitation_sum?.[0]??0} mm</span>
            </div>
          </div>
          <div className="hero-right">
            <div className="hero-icon">{wmo.icon}</div>
            <div style={{marginTop:10}}>
              <div className={`aqi-badge ${aqiLvl.cls}`}>{aqiLvl.emoji} AQI {aqiVal} · {aqiLvl.label}</div>
            </div>
          </div>
        </div>

        {/* WEATHER METRICS */}
        <div className="section-title">🌡️ Weather Variables <span className="pill">Individual Values</span></div>
        <div className="grid-auto" style={{marginBottom:20}}>
          {[
            {icon:"🌡️",label:"Temp (Current)",  val:T(displayTemp),                                     unit:""},
            {icon:"🔻",label:"Temp (Min)",       val:T(d?.temperature_2m_min?.[0]),                     unit:""},
            {icon:"🔺",label:"Temp (Max)",       val:T(d?.temperature_2m_max?.[0]),                     unit:""},
            {icon:"🌧️",label:"Precipitation",   val:d?.precipitation_sum?.[0]??0,                      unit:"mm"},
            {icon:"💧",label:"Humidity",         val:`${curHum??"–"}`,                                  unit:"%"},
            {icon:"☀️",label:"UV Index Max",    val:d?.uv_index_max?.[0]??0,                           unit:"index"},
            {icon:"🌅",label:"Sunrise",          val:fmtTime(d?.sunrise?.[0]),                          unit:"IST"},
            {icon:"🌇",label:"Sunset",           val:fmtTime(d?.sunset?.[0]),                           unit:"IST"},
            {icon:"💨",label:"Max Wind Speed",   val:d?.windspeed_10m_max?.[0]??0,                      unit:"km/h"},
            {icon:"🌂",label:"Precip Prob Max",  val:`${d?.precipitation_probability_max?.[0]??"–"}`,   unit:"%"},
          ].map((m,i)=>(
        ...
        <div className="card" style={{marginBottom:20}}>
          <div className="section-title" style={{marginBottom:10}}>☀️ Sun Cycle</div>
          <div className="sun-row">
            <span>🌅 {fmtTime(d?.sunrise?.[0])}</span>
            <span style={{fontWeight:600}}>{sunPct}% daylight elapsed</span>
        ...
        {/* AIR QUALITY */}
        <div className="section-title">🌬️ Air Quality Metrics <span className="pill">Current Hour</span></div>
        <div className="grid-auto" style={{marginBottom:24}}>
          {[
            {icon:"📊",label:"AQI (EU)",  val:aqiVal,                                  unit:"index"},
            {icon:"🟡",label:"PM10",       val:curPM10!=null?curPM10.toFixed(1):"–",   unit:"μg/m³"},
            {icon:"🟠",label:"PM2.5",      val:curPM25!=null?curPM25.toFixed(1):"–",   unit:"μg/m³"},
            {icon:"💨",label:"CO",         val:curCO  !=null?curCO.toFixed(0)  :"–",   unit:"μg/m³"},
            {icon:"🟤",label:"NO₂",        val:curNO2 !=null?curNO2.toFixed(1) :"–",   unit:"μg/m³"},
            {icon:"🔴",label:"SO₂",        val:curSO2 !=null?curSO2.toFixed(1) :"–",   unit:"μg/m³"},
            {icon:"⚫",label:"CO₂",        val:null,unit:"",na:"Not available via Open-Meteo API"},
          ].map((m,i)=>(
            <div key={i} className="card card-sm metric">
              <div className="metric-icon">{m.icon}</div>
              <div className="metric-label">{m.label}</div>
              {m.na
                ...
                :<><div className="metric-val">{m.val}</div><div className="metric-unit">{m.unit}</div></>
              }
            </div>
          ))}
        </div>
        ...
        {/* HOURLY CHARTS */}
        <div className="section-title">📈 Hourly Visualizations <span className="pill">{date} · Zoom &amp; Pan enabled</span></div>
        <div className="grid-charts">

          <ChartCard title={`🌡️ Temperature (°${unit})`} hint>
            <ReactApexChart type="line" height={220} series={tempSeries}
              options={{
                ...baseOptions({categories:hLabels,unit:`°${unit}`,colors:["#d94f63"],height:220}),
                annotations:{xaxis:[{x:`${nowH}:00`,borderColor:"#1e6fba",
                  label:{text:"Now",style:{color:"#fff",background:"#1e6fba",fontSize:"10px"}}}]},
              }}
            />
          </ChartCard>

          <ChartCard title="💧 Relative Humidity (%)" hint>
            <ReactApexChart type="area" height={220} series={humSeries}
              options={{
                ...baseOptions({categories:hLabels,unit:"%",colors:["#1e6fba"],height:220}),
          ...
          <ChartCard title="🌧️ Precipitation (mm)" hint>
            <ReactApexChart type="bar" height={220} series={precSeries}
              options={baseOptions({categories:hLabels,unit:"mm",colors:["#2e8fd8"],type:"bar",height:220})}
            />
          </ChartCard>

          <ChartCard title="👁️ Visibility (km)" hint>
            <ReactApexChart type="line" height={220} series={visSeries}
              options={baseOptions({categories:hLabels,unit:"km",colors:["#0e8a76"],height:220})}
            />
          </ChartCard>

          <ChartCard title="💨 Wind Speed 10m (km/h)" hint>
            <ReactApexChart type="area" height={220} series={windSeries}
              options={{
                ...baseOptions({categories:hLabels,unit:"km/h",colors:["#7c3aad"],height:220}),
          ...
          <ChartCard title="🌬️ PM10 & PM2.5 (μg/m³)" hint>
            <ReactApexChart type="line" height={220} series={pmSeries}
              options={baseOptions({categories:aqLabels,unit:"μg/m³",colors:["#e07b22","#d94f63"],height:220})}
            />
          </ChartCard>
...
function HistoricalPage({location}) {
  ...
  const [startDate, setStart]  = useState(toDate(new Date(Date.now()-30*864e5)));
  ...
  const load = useCallback(async()=>{
    ...
    if(diff>730){setError("Date range cannot exceed 2 years (730 days).");return;}
    if(diff<1)  {setError("End date must be after start date.");return;}
    setLoading(true); setError(null);
    try{
      const [w,aq]=await Promise.all([
  ...
  (aqData?.hourly?.time||[]).forEach((t,i)=>{
    ...
    if(aqData.hourly.pm10?.[i] !=null) aqMap[day].pm10.push(aqData.hourly.pm10[i]);
    if(aqData.hourly.pm2_5?.[i]!=null) aqMap[day].pm25.push(aqData.hourly.pm2_5[i]);
  });
  const aqDaily=daily.map(d=>({
    date:d.date,
    pm10:aqMap[d.date]?.pm10.length?aqMap[d.date].pm10.reduce((a,b)=>a+b,0)/aqMap[d.date].pm10.length:null,
    pm25:aqMap[d.date]?.pm25.length?aqMap[d.date].pm25.reduce((a,b)=>a+b,0)/aqMap[d.date].pm25.length:null,
  }));
  ...
  const td=thin(daily);const ts=thin(sunDaily);const ta=thin(aqDaily);
  const dates=td.map(d=>d.date);const sDates=ts.map(d=>d.date);const aDates=ta.map(d=>d.date);

  const fmtDecH=(v)=>{if(v==null)return"";const h=Math.floor(v),m=Math.round((v-h)*60);return`${pad2(h)}:${pad2(m)}`;};
  const avgOf  =(arr,fn)=>{const v=arr.map(fn).filter(x=>x!=null);return v.length?(v.reduce((a,b)=>a+b,0)/v.length).toFixed(1):"–";};
  const sumOf  =(arr,fn)=>{const v=arr.map(fn).filter(x=>x!=null);return v.reduce((a,b)=>a+b,0).toFixed(0);};
    ...
    <div>
      <div className="date-row">
        <div>
          ...
          <input type="date" className="date-input" value={startDate}
            min={toDate(twoYrsAgo)} max={endDate} onChange={e=>setStart(e.target.value)}/>
        </div>
        <div>
          <div className="label">End Date</div>
          <input type="date" className="date-input" value={endDate}
            min={startDate} max={toDate(new Date())} onChange={e=>setEnd(e.target.value)}/>
        </div>
        <button className="btn btn-primary" onClick={load}>Analyze Range</button>
        <span style={{fontSize:"0.8rem",color:"var(--muted)",alignSelf:"center"}}>Max range: 2 years</span>
      </div>
      ...
      {!loading&&wData&&(<>
        ...
        <div className="grid-auto" style={{marginBottom:22}}>
          {[
            ...
            {icon:"🌡️",label:"Avg Temp",     val:avgOf(daily,   d=>d.mean),     unit:"°C"},
            {icon:"🌧️",label:"Total Precip", val:sumOf(daily,   d=>d.precip),   unit:"mm"},
            {icon:"💨",label:"Avg Max Wind",  val:avgOf(daily,   d=>d.windMax),  unit:"km/h"},
            {icon:"🟡",label:"Avg PM10",      val:avgOf(aqDaily, d=>d.pm10),    unit:"μg/m³"},
            {icon:"🟠",label:"Avg PM2.5",     val:avgOf(aqDaily, d=>d.pm25),    unit:"μg/m³"},
          ].map((m,i)=>(
            <div key={i} className="card card-sm metric">
              <div className="metric-icon">{m.icon}</div>
              <div className="metric-label">{m.label}</div>
        ...
        <div className="hist-section">
          ...
          <ChartCard title="Mean / Max / Min Temperature (°C)" hint>
            <ReactApexChart type="line" height={240}
              series={[
                {name:"Mean",data:td.map(d=>d.mean  !=null?+d.mean.toFixed(1)  :null)},
                {name:"Max", data:td.map(d=>d.max   !=null?+d.max.toFixed(1)   :null)},
                {name:"Min", data:td.map(d=>d.min   !=null?+d.min.toFixed(1)   :null)},
              ]}
              options={{
                ...baseOptions({categories:dates,unit:"°C",colors:["#1e6fba","#d94f63","#2e8fd8"],height:240}),
        ...
        <div className="hist-section">
          ...
          <ChartCard title="Sunrise & Sunset Times (24h)" hint>
            <ReactApexChart type="line" height={220}
              ...
              options={{
                ...baseOptions({categories:sDates,colors:["#f5a623","#e07b22"],height:220}),
                yaxis:{
                  min:4,max:22,
                  labels:{formatter:fmtDecH,style:{fontSize:"11px",colors:"#5a7d99"}},
                  title:{text:"Time (24h)",style:{color:"#5a7d99",fontSize:"11px"}},
                },
                tooltip:{y:{formatter:v=>fmtDecH(v)+" IST"},theme:"light"},
              }}
            />
          </ChartCard>
        </div>
        ...
        <div className="hist-section">
          ...
          <ChartCard title="Daily Total Precipitation (mm)" hint>
            <ReactApexChart type="bar" height={220}
              series={[{name:"Precipitation",data:td.map(d=>d.precip!=null?+d.precip.toFixed(1):0)}]}
              options={baseOptions({categories:dates,unit:"mm",colors:["#2e8fd8"],type:"bar",height:220})}
            />
          </ChartCard>
        </div>
        ...
        <div className="hist-section">
          ...
          <div className="grid-charts">
            <ChartCard title="Max Wind Speed (km/h)" hint>
              <ReactApexChart type="line" height={220}
                series={[{name:"Max Wind",data:td.map(d=>d.windMax!=null?+d.windMax.toFixed(1):null)}]}
                options={{
                  ...baseOptions({categories:dates,unit:"km/h",colors:["#7c3aad"],height:220}),
              ...
              <div className="wind-dir-grid">
                {td.map((d,i)=>(
                  <div key={i} className="wind-dir-item">
                    <div className="dir-badge" title={`${d.date}: ${d.windDir}°`}>
        ...
        <div className="hist-section">
          ...
          <ChartCard title="PM10 & PM2.5 Daily Average (μg/m³)" hint>
            <ReactApexChart type="line" height={230}
              series={[
                {name:"PM10",  data:ta.map(d=>d.pm10!=null?+d.pm10.toFixed(2):null)},
                {name:"PM2.5", data:ta.map(d=>d.pm25!=null?+d.pm25.toFixed(2):null)},
              ]}
              options={baseOptions({categories:aDates,unit:"μg/m³",colors:["#e07b22","#d94f63"],height:230})}
            />
          </ChartCard>
        </div>
      </>)}
      ...
      {!loading&&!wData&&!error&&(
        <div className="empty-state">
          ...
          <div className="empty-title">Select a date range and click Analyze Range</div>
          <div className="empty-sub">Explore up to 2 years of historical weather &amp; air quality data</div>
        </div>
      )}
    </div>
  );
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      ()=>{setGeoMsg("Location access denied — using New Delhi as fallback.");setLocation({lat:28.6139,lon:77.209});},
      {timeout:9000}
    );
  },[]);
  ...
  return (
    <>
      ...
      <div className="app">
        <nav className="nav">
  <div className="brand">Sky<span>Lens</span></div>
  ...
  <div className="nav-tabs">
    ...
    >
      📊 Historical Analysis
        ...
        <main className="main">
          {geoMsg&&<div className="error-box" style={{marginBottom:14}}>📍 {geoMsg}</div>}
